...
namespace libnlp {
    ...
    namespace internal {
        ...
        template<int>
        size_t FNVHash(const char *text, const size_t byteLength);
    ...
    public:
        ...
        std::string to_string() const { return std::string(str, str + byteLength); }
        ...
        LengthType FindBytePosition(const UTF8StringSliceBase &pattern) const {
            return static_cast<LengthType>(
                    to_string().find(pattern.str, 0, pattern.byteLength));
        }
