...
namespace libnlp::dict {
    ...
    protected:
        ...
        text_dict_ptr CreateTextDictForText() const {
            ...
            lex->add(dict_entry_factory::create("BYVoid", "byv"));
            lex->add(dict_entry_factory::create("zigzagzig", "zag"));
            lex->add(dict_entry_factory::create(utf8("積羽沉舟"), utf8("羣輕折軸")));
            lex->add(dict_entry_factory::create(utf8("清"), "Tsing"));
            lex->add(dict_entry_factory::create(utf8("清華"), "Tsinghua"));
            lex->add(dict_entry_factory::create(utf8("清華大學"), "TsinghuaUniversity"));
            lex->sort();
        ...
        basic_dict_ptr CreateDictForCharacters() const {
            lexicon_ptr lex(new lexicon);
            lex->add(dict_entry_factory::create(
        ...
        basic_dict_ptr CreateDictForPhrases() const {
            lexicon_ptr lex(new lexicon);
            lex->add(dict_entry_factory::create(utf8("太后"), utf8("太后")));
            lex->add(dict_entry_factory::create(utf8("头发"), utf8("頭髮")));
            lex->add(dict_entry_factory::create(utf8("干燥"), utf8("乾燥")));
            lex->add(dict_entry_factory::create(utf8("鼠标"), utf8("鼠標")));
            lex->sort();
            return text_dict_ptr(new text_dict(lex));
        }
        ...
        basic_dict_ptr CreateDictForTaiwanVariants() const {
            lexicon_ptr lex(new lexicon);
            lex->add(dict_entry_factory::create(utf8("裏"), utf8("裡")));
            return text_dict_ptr(new text_dict(lex));
        }

        basic_dict_ptr CreateTaiwanPhraseDict() const {
            lexicon_ptr lex(new lexicon);
            lex->add(dict_entry_factory::create(utf8("鼠标"), utf8("滑鼠")));
            lex->add(dict_entry_factory::create(utf8("服务器"), utf8("伺服器")));
            lex->add(dict_entry_factory::create(utf8("克罗地亚"), utf8("克羅埃西亞")));
            lex->sort();
            return text_dict_ptr(new text_dict(lex));
        }
        ...
        void TestMatch(const basic_dict_ptr &dict) const {
            std::optional<const dict_entry *> entry;
        ...
        void TestMatchAllPrefixes(const basic_dict_ptr &dict) const {
            const std::vector<const dict_entry *> matches =
